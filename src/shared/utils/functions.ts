...
export const isJSONContent = function (jsonContent: string) {
...
export const toggleValue = (toggleValues: any[], currentValue: any, step = 1): any => {
  return (
    (toggleValues = asArray(toggleValues)) &&
    toggleValues[
      (toggleValues.findIndex((val) => val === currentValue) + step) % toggleValues.length
    ]
  )
}
export const nextValue = (toggleValues: any[], currentValue: any, step = 1): any =>
  toggleValue(toggleValues, currentValue, 1 * step)

export const prevValue = (toggleValues: any[], currentValue: any, step = 1): any =>
  toggleValue(toggleValues, currentValue, -1 * step)
...
export const limitNumber = (function () {
  ...
  const limitNumber = function (...args) {
    ...
    if (Array.isArray(numbers)) {
      for (let d = numbers, i = 0, l = d.length; i < l; i++) {
...
export const hashMap = function (object, fn) {
  if (typeof object === 'object' && !isArray(object))
    for (const k in object) object[k] = fn(object[k], k)
...
export const toSnakeCase = (s) => {
  const words = splitCaseString(s)
  for (let i = 0, l = words.length; i < l; i++) words[i] = words[i].toLowerCase()
  return words.join('_')
}
